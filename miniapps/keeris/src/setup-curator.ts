...
const prisma: any = await provisionSqliteDb(databaseName, false, { databasePath });
...
console.log(JSON.stringify({ phase: 'setup-complete', databasePath, plugins: engine.plugins.map((plugin: any) => plugin.name), tools: [...engine.tools.keys()], workflows: [...engine.agents.keys()] }, null, 2));
