...
- **Agentic Scripting Engine**: We use a custom tool-chaining architecture. See [.agent/AGENTIC_SCRIPTING.md](file:///z:/curator/.agent/AGENTIC_SCRIPTING.md) for detailed builder instructions and tool inventory.
- **Prompt Management**: System instructions are managed via the `Script` model (formerly `PromptTemplate`), which supports both static prompts and dynamic JS-based tool chains.
...
## Workflows
- **Running a Script**: Use the `execute_script` tool with either a `scriptName` or an inline `body`.
- **Parallel Processing**: Use `ToolChain.spawn()` to process multiple items concurrently via child Requests.
- **Data Inheritance**: Child Requests automatically inherit resources and context from their parents, allowing for recursive RAG.
