...
CREATE TABLE IF NOT EXISTS "ResourceTree" (
    "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE IF NOT EXISTS "_RequestResources" (
    "A" INTEGER NOT NULL,
    "B" INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS "_ConversationResources" (
    "A" INTEGER NOT NULL,
    "B" INTEGER NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS "Resource_uri_key" ON "Resource"("uri");
CREATE UNIQUE INDEX IF NOT EXISTS "Resource_id_existent_key" ON "Resource"("id", "existent");
CREATE UNIQUE INDEX IF NOT EXISTS "_RequestResources_AB_unique" ON "_RequestResources"("A", "B");
CREATE INDEX IF NOT EXISTS "_RequestResources_B_index" ON "_RequestResources"("B");
CREATE UNIQUE INDEX IF NOT EXISTS "_ConversationResources_AB_unique" ON "_ConversationResources"("A", "B");
CREATE INDEX IF NOT EXISTS "_ConversationResources_B_index" ON "_ConversationResources"("B");
`;

export const SEED_SQL = `
INSERT OR IGNORE INTO "User" (id, email, name, createdAt, updatedAt) 
VALUES ('curator-user', 'curator@arula.dev', 'curator', strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'));

INSERT OR IGNORE INTO "Project" (id, name, userId, existent, createdAt, updatedAt)
VALUES ('system', 'System Project', 'curator-user', 1, strftime('%Y-%m-%d %H:%M:%f', 'now'), strftime('%Y-%m-%d %H:%M:%f', 'now'));
