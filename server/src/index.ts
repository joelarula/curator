...
import { syncToolsToDatabase } from './services/ToolRegistry.js';
...
async function startServer() {
  ...
  // System Dashboard (HTML Visualization)
  app.get('/dashboard', (_req, res) => {
    res.type('html').send(metricsService.getDashboardHTML());
  });
  ...
  // Sync registered tools to DB lookup table before binding port
  await syncToolsToDatabase(prisma);

  app.listen(PORT, () => {
    console.log(`🚀 Server ready at http://localhost:${PORT}`);
    console.log(`📊 GraphQL IDE: http://localhost:${PORT}/graphql`);
    console.log(`📈 Metrics Dashboard: http://localhost:${PORT}/dashboard`);
